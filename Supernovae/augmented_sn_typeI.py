...
CSV_PATH = Path(
    ...
    "filtered_specific_type_I_supernovae_with_label.csv"
)
OUT_PATH = CSV_PATH.with_name(CSV_PATH.stem + "_augmented.csv")

KEEP_COLS = ["Gi", "GVhel", "mag", "Type", "Type I Label"]
NUMERIC = ["Gi", "GVhel", "mag"]  # numeric cols
TARGET = "Gi"
...
# convert numeric cols; bad parses become NaN → dropped next line
for col in NUMERIC:
...
# decide how many synthetic rows we need
...
sigma = (y - reg.predict(X)).std(ddof=1)  # residual std-dev

rng = np.random.default_rng(SEED)

# 6. generate synthetic rows
synthetic_rows = []
for _ in range(N_SYN):
    base = df.sample(1, random_state=rng).iloc[0]  # conditioning row
...
print(f"Augmented file written to     : {OUT_PATH}")
